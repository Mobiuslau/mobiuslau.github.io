...
### 1.3. &nbsp; Deck Properties and Play Considerations
...
{% include image_inline.html side="right" width="49%" label="op15-amy-v16" src="lists/gy-law_op15_amy_16.png" %}
...
{% include image_inline.html side="left" width="49%" label="op15-shimona-no-borsa" src="lists/gy-law_op15_shimona_no_borsa.png" %}
<b style="font-size: 16px;">🡮 Shimona's Consistency Without [Borsa](#borsa)</b><br>
...
{% include image_inline.html side="right" width="49%" label="op15_swagzaur_ily" src="lists/gy-law_op15_swagzaur_ily.png" %}
<b style="font-size: 16px;">🡮 Swagzaur's Starve With [ILY](#ily)</b><br>
🡪 <a target="_blank" href="lists/gy-law_op15_swagzaur_ily.deck.txt"><em>Text List.</em></a>

"This list was designed to starve the opponent by continuously applying moderate to high pressure for as long as necessary. [6c Law](#6c) is used in conjunction with LA as the main cornerstone of quickly expanding the board at any stage, with [7c Law](#7c) being a taller option for games that are expected to reach the late stage. [DMG](#dmg) is included as a way to clear mid-game boards for MUs where removing those bodies is essential, also serving to push for lethal if the opponent is turtled behind blockers. [ILY](#ily) and [4c Law](#promo-law) are both maxed out as the main engine that guarantees consistent survivability and board protection via counter, [6c Law](#6c) and [8c FemLaw](#femlaw) both serving as secondary targets for [ILY](#ily), [6c Law](#6c) as a `Blocker` and [8c FemLaw](#femlaw) as a healer that also applies pressure with its access to the `Rush` keyword. [10c Lepo](#lepo) rounds up the list as the boss character that fortifies the wide mid-sized board so it can both threaten tall bodies and ascertain dominance over wide mid-sized boards."
<br clear="all"><br>
