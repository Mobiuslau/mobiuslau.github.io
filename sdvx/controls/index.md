...
|Function|Vivid Wave|Exceed Gear Konasute|Description|
|:-|:-|:-|:-|
|Select option/folder/chart|Start|Start||
|Leave folder|FX-L|FX-L||
|Change Sort|FX-R|FX-R|EG has more options than VW.|
|Select chart|Know-R|Know-R||
|Fast scroll|FX-L + knob-R|FX-L + knob-R|Scrolls by catagories depending on sort.|
|Select difficulty|Knob-L|Knob-L||
|Toggle options menu|FX-L + FX-R|FX-L + FX-R||
|Navigate options menu|FX-L, FX-R|Knob-L||
|Change option|Knob-#|Knob-R||
|Save mmod speed|N.A.|num-6|Changes modspeed number.|
|Toggle Rival leaderboards|num-8|N.A.|Only shows 4 rivals, and the top-1 score.
|Toggle favourite chart|num-0|num-0||
|Random Chart|num-3|num-3||
...
There is no waiting lobby when using Premium Time. These controls should also function during gameplay. Note that Exceed Gear Konasute uses mmod by default.

|Function|Vivid Wave|Exceed Gear Konasute|Description|
|:-|:-|:-|:-|
|Change xmod lane speed|Hold Start + knob|N.A.|Changes multiplier with BPM.|
|Change mmod lane speed|Tap then hold start + knob|Hold Start + knob|Changes modspeed number.|
|Force constant modspeed|Start + num-6|N.A.|Red number bottom left.|
|Visual offset|Start + num-9, num-3|Start + num-9, num-3|Blue number bottom left. + if early, - if late.|
|Audio offset w.r.t judgement|Start + num-8, num-2|Start + num-8, num-2|Yellow number bottom left. + if hitting early, - if hitting late.|
|Headphone volume|Start + num-7, num-1|Start + num-7, num-1|Bottom right gauge.|
